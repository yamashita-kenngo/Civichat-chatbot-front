...
const getUserFav = async (userId: SetStateAction<string>, setUserFav: { (value: SetStateAction<boolean>): void; (arg0: boolean): void; }) => {
  ...
  let userList: string[] = []
  for await (const fav of await res.json()){
    console.log(fav)
    await fetch(
      `https://oci2.keitan.dev/info/${fav}`,
      {
        method: "GET"
      }
    ).then(response => userList.push(response))
  }
  console.log(JSON.stringify(userList))
  setUserFav(userList.join(','))
}
...
const OthersFromId: NextPage<Props> = ({
  ...
  const [result, setUserFav] = useState("");
      ...
      liff.init({ liffId: process.env.NEXT_PUBLIC_LIFFID }).then(() => {
        if(!liff.isLoggedIn()){
          ...
          liff.getProfile().then(async (profile: { userId: SetStateAction<string>; }) => {
            getUserFav(profile.userId, setUserFav)
            console.log(true)
          });
        }
      })
  ...
  return (
    <div className="container items-center justify-center mx-auto px-2">
      ...
      <div>
        <h3 className="text-xl px-5 font-bold text-2xl">一覧</h3>
        {result.split(',').map((system) => {
